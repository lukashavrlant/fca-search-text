...
Pomocí této věty už můžeme napsat algoritmus \ref{alg.upper}, kterým vygenerujeme všechny horní koncepty konceptu $\addsp{G, M}$ ve svazu $\lattice$.
...
\Function{UpperNeighbors}{$\addsp{G, M}, \lattice$}
    ...
    \ForAll{$g \in X\setminus G$} 
    	\State $M_1\gets(G\cup\adds{g})\up$
    	\State $G_1\gets M_1\down$
    	\If{$candidates \cap (G_1\setminus G\setminus\adds{g}) = \emptyset$}
    		\State $neighbors\gets neighbors \cup(\addsp{G_1, M_1})$
    	\Else
