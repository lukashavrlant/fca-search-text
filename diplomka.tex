...
\begin{theorem}[Systém pevných bodů uzávěrového operátoru]
...
\begin{enumerate}
	...
	\begin{eqnarray}
	\biginf_{j\in J}\addsp{A_j, B_j}&=&\addsp{\bigcap_{j\in J}A_j, \addk{\bigcup_{j\in J}B_j}\downup},\\
	...
	\begin{itemize}
		\item $\gamma(X)$ je supremálně husté ve $V$ a $\mu(Y)$ je infomálně husté v $V$,
		\item $\gamma(x)\le\mu(y)$ právě tehdy, když $\addsp{x,y}\in I$.
	\end{itemize}
\end{enumerate}
\end{theorem}

První část nám říká, jak hledat suprema a infima množiny konceptů. Chceme-li najít 
