...
%cDiplomová práce je rozdělena do několika kapitol. V první kapitole, \uv{Information retrieval}, je popsáno, jakým způsobem pracuje samotné jádro vyhledávače. 
...
První vlastnost říká, původní útvar bude vždy podmnožinou uzavřeného útvaru. Původní rovinný útvar bude vždy celý obsažen v konvexním útvaru, který ho má obalovat -- uzavřením pouze \uv{přidáme} původnímu útvaru body tak, aby vznikl konvexní útvar. Druhá vlastnost říká, že pokud máme dva různé útvary, přičemž jeden je zanořený ve druhém, tak i jejich obaly budou zanořené. Poslední vlastnost říká, že obalem obalu je tentýž obal. Pokud obalíme hvězdu konvexním útvarem, pro příklad pravidelným pětiúhelníkem, tak obalem tohoto pětiúhelníku bude tentýž pětiúhelník.
...
Myšlenka algoritmu je založena na tom, že když přidáme k extentu $G$ konceptu $\addsp{G, M}$ objekt z $X$, který není v $G$, a uzavřeme, získáme koncept, který je určitě větší. Pokud postupně přidáme všechny objekty z $X \setminus G$, dostaneme množinu konceptů, která obsahuje všechny horní sousedy. Množinu si pojmenujeme $S$:
$$
S=\adds{\addsp{(G\cup \adds{g})\updown, (G\cup \adds{g})\up}\sep g\in X\setminus G}
$$
Tato množina sice obsahuje všechny horní sousedy, ale bohužel i některé navíc. Může se stát, že přidáním objektu $g\in X\setminus G$ \uv{přeskočíme} nějaký koncept a dostaneme koncept, který je o více než jednu úroveň výše. Abychom tak získali pouze množinu horních sousedů, musíme při každém přidání nového objektu zkontrolovat, zda jsme skutečně vygenerovali horního souseda. 

To zkontrolujeme tak, že zjistíme, zda všechny objekty, které přibyly v novém konceptu $\addsp{G', M'}$, generují tentýž koncept. Přesněji to popisuje následující věta.

\begin{theorem}[Jak nalézt horní sousedy]
Nechť $\addsp{G, M} \in \lattice$ a $\addsp{G, M}\ne\addsp{X, X\up}$. Pak $(G\cup\adds{g})\updown$, kde $g\in X\setminus G$ je extent horního souseda konceptu $\addsp{G, M}$ právě tehdy, když pro všechna $x\in(G\cup\adds{g})\updown\setminus G$ platí $(G\cup\adds{x})\updown=(G\cup\adds{g})\updown$.
\end{theorem}

Pomocí této věty už můžeme napsat algoritmus, kterým vygenerujeme všechny horní koncepty konceptu $\addsp{G, M}$ ve svazu $\lattice$, viz obrázek \ref{alg.upper}

\begin{figure} \label{alg.upper}
\begin{algorithmic}[1]
\Function{UpperNeighbors}{$\addsp{G, M}, \lattice$}
	\State $candidates \gets X \setminus G$
    \State $neighbors \gets \emptyset$
    \ForAll{$g \in X\setminus G$} \label{alg.upper.getg}
    	\State $M'\gets(G\cup\adds{g})\up$
    	\State $G'\gets M'^{\downarrow}$
    	\If{$candidates \cap (G'\setminus G\setminus\adds{g}) = \emptyset$}\label{alg.upper.if}
    		\State $neighbors\gets neighbors \cup(\addsp{G', M'})$
    	\Else
    		\State $candidates\gets candidates\setminus\adds{g}$
    	\EndIf
    \EndFor
...
% \ssection{Příklad}
% \sssection{Information retrieval část}
% Ukážeme si postup vyhledávače na příkladu. Mějme sadu zaindexovaných dokumentů $\alldoc$, kde bude $100$ článků s radami ohledně podnikání. Položme vyhledávači dotaz $Q=$\uv{Jak podnikat, jsem student?}. Jako první vyhledávač aplikuje textové transformace popsané v kapitole \ref{prepr}. Tj. převede text na malá písmena, převede na stemy, odstraní diakritiku a podobně. Na konci těchto transformací dostaneme dotaz \uv{podnik AND student}. 

% Vyhledávače pomocí svého indexu nalezne dokumenty, které obsahují slovo \uv{podnik}. Dále nalezne dokumenty, které obsahují slovo \uv{student}. Protože výchozí logická spojka je AND, udělá vyhledávač průnik mezi oběma množinami, čímž získáme množinu dokumentů, která odpovídá našemu zadanému dotazu, množinu $D$. Těchto dokumentů je $|D|=23$.

% V dalším kroku tyto dokumenty seřadíme dle relevance. K tomu použijeme $\tfidf$ funkci. Pro každý dokument spočítáme $\tfidf$ skóre pro obě slova zvlášť a tyto hodnoty sečteme. Upravíme skóre, pokud se klíčová slova vyskytují v titulku, URL nebo popisku. To je konec klasické části vyhledávače, plynule přejdeme do FCA části. 

% \sssection{FCA část}
% Náš dotaz měl velikost dva, $|\beta(Q)|=2$, takže jako první provedeme druhý dotaz do vyhledávače. Dotaz bude mít tvar $Q'=$\uv{podnikat OR student}. Vyhledávač nám vrátí novou sadu dokumentů, $D'$, kterých je $|D'|=68$.

% V dalším kroku sestavíme kontext. Máme nastaveno omezení na $50$ dokumentů, takže vezmeme prvních $50$ dokumentů z $D'$. To bude tvořit množinu objektů $X'$ rozšířeného kontextu $\addsp{X', Y', I'}$. 
...
\ssection{Vyhodnocení výsledků}
Kvalita výsledků v prvé řadě závisí na kvalitě zpracovávaných dokumentů. Některé další aspekty lze ovlivnit nastavením vyhledávače. 

\sssection{Kvalita obsahů dokumentů}
...
Když pak uživatel vyhledá jméno nějakého zaměstnance z katedry, často se mezi prvními výsledky zobrazí právě aktuality nebo kalendář, protože samotná stránka není příliš velká, aby to srazilo hodnocení $\tfidf$ a obvykle dané jméno obsahuje hodněkrát. Podobný výsledek se objeví, pokud vyhledáme slovo \uv{seminář}. Pokud se seminář koná dvacetkrát do roka, má v kalendáři dvacet různých záznamů a kalendář tak obsahuje minimálně dvacetkrát slovo \uv{seminář}. V hodnocení pak snadno předběhne stránku, která je pro informatický seminář přímo určena. Tyto problémy lze vyřešit různými způsoby:
...
Teoreticky by vyhledávač mohl získávat atributy dokumentů z elementu \code{<META NAME=keywords>}, který je k tomu určen. Ale během historie webu byl tento element poměrně hodně zneužívám vkládáním nesmyslných klíčových slov jako \uv{sex, porno, nahá, pamela, Anderson}, takže tento element postupně běžné vyhledávače začali téměř ignorovat a většina běžných webů už ho ani nepoužívá. \name{} ho tak zcela ignoruje. 
...
Web musí mít správně určené kódování stránek prostřednictvím elementu \code{<meta http-equiv='content-type'>}. Pokud se tento element nenajde, bude se předpokládat, že web je v kódování \code{utf-8}. Vyhledávač nezkoumá http hlavičky, které server spolu se stránkou poslal. 

\sssection{Některé nedostatky vyhledávače}
